...
pub fn generate_global_types() -> Result<()> {
    let types_dir = project_root().join("xtask/codegen/src/TypeScript/src/lib");
    let mut global_types = Vec::new();
    let files = fs::read_dir(&types_dir)?;
    for file in files {
        let path = file?.path();
        println!("parsing {:?}", path);
        if path.to_str().unwrap_or_default().ends_with("d.ts") {
    ...
    println!(
        "Found {} unique types (after deduplication)",
        global_types.len()
    );
    ...
    update(&output_path, &output, &Mode::Overwrite)?;

    println!(
        "Generated generated_globals.rs with {} types",
        global_types.len()
    );
...
fn process_root(root: &AnyJsRoot, file_name: &str, global_types: &mut Vec<GlobalType>) {
    ...
    // TODO: Per https://github.com/biomejs/biome/pull/6143, biome supports nested type qualifiers.
    // We should rely on this for more robust type awareness.
...
fn process_module_item(
    ...
    file_name: &str,
    global_types: &mut Vec<GlobalType>,
) {
    match item {
